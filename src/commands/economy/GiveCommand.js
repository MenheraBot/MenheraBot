...
module.exports = class GiveCommand extends Command {
  ...
  async giveStar(user, user2, valor, ctx, mencionado) {
    if (valor > user.estrelinhas)
      return ctx.reply(
        'error',
        `${ctx.locale('commands:give.poor')} ${ctx.locale('commands:give.stars')}`,
      );
    ...
    ctx.reply(
      'success',
      `${ctx.locale('commands:give.transfered', { value: valor, emoji: '⭐' })} ${mencionado}`,
    );
  }

  async giveDemon(user, user2, valor, ctx, mencionado) {
    if (valor > user.caçados)
      return ctx.reply(
        'error',
        `${ctx.locale('commands:give.poor')} ${ctx.locale('commands:give.demons')}`,
      );
    ...
    ctx.reply(
      'success',
      `${ctx.locale('commands:give.transfered', {
        value: valor,
        emoji: '<:Demon:758765044443381780>',
      })} ${mencionado}`,
    );
  }
  ...
  async giveAngel(user, user2, valor, ctx, mencionado) {
    if (valor > user.anjos)
      return ctx.reply(
        'error',
        `${ctx.locale('commands:give.poor')} ${ctx.locale('commands:give.angels')}`,
      );
    ...
    ctx.reply(
      ...
      `${ctx.locale('commands:give.transfered', {
        value: valor,
        emoji: '<:Angel:758765044204437535>',
      })} ${mencionado}`,
    );
  }

  async giveSD(user, user2, valor, ctx, mencionado) {
    if (valor > user.semideuses)
      return ctx.reply(
        'error',
        `${ctx.locale('commands:give.poor')} ${ctx.locale('commands:give.semigods')}`,
      );
    ...
    ctx.reply(
      ...
      `${ctx.locale('commands:give.transfered', {
        value: valor,
        emoji: '<:SemiGod:758766732235374674>',
      })} ${mencionado}`,
    );
  }

  async giveGod(user, user2, valor, ctx, mencionado) {
    if (valor > user.deuses)
      return ctx.reply(
        'error',
        `${ctx.locale('commands:give.poor')} ${ctx.locale('commands:give.gods')}`,
      );
  ...
  async run(ctx) {
    ...
    const user2 = await this.client.database.repositories.userRepository.findOrCreate(
      mencionado.id,
    );
