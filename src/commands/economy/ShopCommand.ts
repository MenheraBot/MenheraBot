  ...
  collector.on('collect', (m) => {
    ...
    if (m.content === '1') {
        ...
        fields: [
          {
            ...
            value: coresDisponíveis
              .map(
                (c) =>
                  `${c.nome} | ${ctx.locale('commands:shop.dataCores_fields.color_code')} \`${
                    c.cor
                  }\` | ${ctx.locale('commands:shop.dataCores_fields.price')} **${c.preço}**⭐`,
              )
              .join('\n'),
            inline: false,
          },
      ...
      CorColetor.on('collect', async (msg) => {
        if (!validCorArgs.some((answer) => answer.toLowerCase() === msg.content.toLowerCase()))
          return ctx.replyT('error', 'commands:shop.invalid-option');
        let choice;
        switch (msg.content) {
          case '1':
            if (ctx.data.user.cores.some((res) => res.cor === coresDisponíveis[0].cor))
              return ctx
                .replyT('yellow_circle', 'commands:shop.buy_colors.has-color')
                .then(() => embedMessage.delete().catch());
            if (ctx.data.user.estrelinhas < coresDisponíveis[0].preço)
              return ctx
            ...
            break;
          case '2':
            if (ctx.data.user.cores.some((res) => res.cor === coresDisponíveis[1].cor))
              return ctx
                .replyT('yellow_circle', 'commands:shop.buy_colors.has-color')
                .then(() => embedMessage.delete().catch());
            if (ctx.data.user.estrelinhas < coresDisponíveis[1].preço)
              return ctx
                .replyT('error', 'commands:shop.buy_colors.poor')
            ...
            break;
          case '3':
            if (ctx.data.user.cores.some((res) => res.cor === coresDisponíveis[2].cor))
              return ctx
                .replyT('yellow_circle', 'commands:shop.buy_colors.has-color')
                .then(() => embedMessage.delete({ timeout: 500 }).catch());
            if (ctx.data.user.estrelinhas < coresDisponíveis[2].preço)
              return ctx
                .replyT('error', 'commands:shop.buy_colors.poor')
                .then(() => embedMessage.delete({ timeout: 500 }).catch());
            choice = 2;
            break;
          case '4':
            if (ctx.data.user.cores.some((res) => res.cor === coresDisponíveis[3].cor))
              return ctx
                .replyT('yellow_circle', 'commands:shop.buy_colors.has-color')
                .then(() => embedMessage.delete({ timeout: 500 }).catch());
            if (ctx.data.user.estrelinhas < coresDisponíveis[3].preço)
              return ctx
                .replyT('error', 'commands:shop.buy_colors.poor')
                .then(() => embedMessage.delete({ timeout: 500 }).catch());
            choice = 3;
            break;
          case '5':
            if (ctx.data.user.cores.some((res) => res.cor === coresDisponíveis[4].cor))
              return ctx
                .reply('yellow_circle', 'commands:shop.buy_colors.has-color')
                .then(() => embedMessage.delete({ timeout: 500 }).catch());
            if (ctx.data.user.estrelinhas < coresDisponíveis[4].preço)
              return ctx
                .replyT('error', 'commands:shop.buy_colors.poor')
                .then(() => embedMessage.delete({ timeout: 500 }).catch());
            choice = 4;
            break;
          case '6':
            if (ctx.data.user.cores.some((res) => res.cor === coresDisponíveis[5].cor))
              return ctx
                .replyT('yellow_circle', 'commands:shop.buy_colors.has-color')
                .then(() => embedMessage.delete({ timeout: 500 }).catch());
            if (ctx.data.user.estrelinhas < coresDisponíveis[5].preço)
              return ctx
                .replyT('error', 'commands:shop.buy_colors.poor')
                .then(() => embedMessage.delete({ timeout: 500 }).catch());
            choice = 5;
          ...
          case '7': {
            if (ctx.data.user.cores.some((res) => res.nome.startsWith('7')))
              return ctx
                .replyT('yellow_circle', 'commands:shop.buy_colors.has-color')
                .then(() => embedMessage.delete({ timeout: 500 }).catch());
            if (ctx.data.user.estrelinhas < coresDisponíveis[6].preço)
              return ctx
                .replyT('error', 'commands:shop.buy_colors.poor')
                .then(() => embedMessage.delete({ timeout: 500 }).catch());
            choice = 6;
            ...
            hexColletor.on('collect', (hexMsg: Message) => {
              ...
              if (isHexColor(hexMsg.content)) {
                ...
                ctx
                  .replyT('sucess', 'commands:shop.buy_colors.yc-confirm', {
                    color: hexMsg.content,
                    price: coresDisponíveis[6].preço,
                    stars: ctx.data.user.estrelinhas - coresDisponíveis[6].preço,
                  })
                  .then(() => embedMessage.delete().catch);
              } else {
                return ctx
        ...
        if (choice !== 6) {
          await repo.update(ctx.message.author.id, {
            $inc: { estrelinhas: -coresDisponíveis[choice].preço },
            $push: { cores: coresDisponíveis[choice] },
          });
          ctx
            .replyT('success', 'commands:shop.buy_colors.buy-success', {
              name: coresDisponíveis[choice].nome,
              price: coresDisponíveis[choice].preço,
              stars: ctx.data.user.estrelinhas - coresDisponíveis[choice].preço,
            })
            .then(() => embedMessage.delete().catch());
        }
      });
