...
module.exports = class CooldownsCommand extends Command {
  ...
  async run(ctx) {
    ...
    if (userRpg) {
      if (deathTimeInMilis > 0) txt += `\`${ctx.locale('commands:cooldowns.death')}\` | ${deathTimeInMilis > 3600000 ? `**${moment.utc(deathTimeInMilis).format('HH:mm:ss')}** ${ctx.locale('commands:cooldowns.hours')}` : `**${moment.utc(deathTimeInMilis).format('mm:ss')}** ${ctx.locale('commands:cooldowns.minutes')}`}\n`;
      if (hotelTimeInMilis > 0) txt += `\`${ctx.locale('commands:cooldowns.hotel')}\` | ${hotelTimeInMilis > 3600000 ? `**${moment.utc(hotelTimeInMilis).format('HH:mm:ss')}** ${ctx.locale('commands:cooldowns.hours')}` : `**${moment.utc(hotelTimeInMilis).format('mm:ss')}** ${ctx.locale('commands:cooldowns.minutes')}`}\n`;
      ...
      jobCooldownInMilis < 0 ? txt += `\`${ctx.locale('commands:cooldowns.job')}\` | ${ctx.locale('commands:cooldowns.no-cooldown')}\n` : txt += `\`${ctx.locale('commands:cooldowns.job')}\` | ${jobCooldownInMilis > 3600000 ? `**${moment.utc(jobCooldownInMilis).format('HH:mm:ss')}** ${ctx.locale('commands:cooldowns.hours')}` : `**${moment.utc(jobCooldownInMilis).format('mm:ss')}** ${ctx.locale('commands:cooldowns.minutes')}`}\n`;
    }
