...
module.exports = class Canvas {
  /*  static async NewProfileImage({
    member, user, avatar, marry,
  }, t) {
    // Criação da Área de Trabalho
    const canvas = CanvasImport.createCanvas(1080, 720);
    const ctx = canvas.getContext('2d');
    CanvasImport.registerFont(path.resolve('src', 'assets', 'fonts', 'quicksand.ttf'), { family: 'MenheraProfile' });

    // Plano de Fundo
    const background = await CanvasImport.loadImage('https://i.imgur.com/9uDplMg.png');
    ...
    // Nick do usuário
    ctx.fillStyle = '#fff';
    ctx.font = 'bold 48px MenheraProfile';
