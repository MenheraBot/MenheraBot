...
module.exports = class NewHttp {
  static async astolfoRequest(text) {
    let err = false;
    const data = await request.get('/astolfo', { data: { text } }).catch(() => {
      err = true;
    });
  ...
  static async philoRequest(text) {
    let err = false;
    const data = await request.get('/philo', { data: { text } }).catch(() => {
      err = true;
    });
    const returnData = { err, data: data?.data };
    return returnData;
  ...
  static async shipRequest(linkOne, linkTwo, shipValue) {
    let err = false;
    const data = await request.get('/ship', { data: { linkOne, linkTwo, shipValue } }).catch(() => {
      err = true;
    });
    const returnData = { err, data: data?.data };
    return returnData;
  ...
  static async trisalRequest(userOne, userTwo, userThree) {
    ...
    const data = await request
      .get('/trisal', { data: { userOne, userTwo, userThree } })
      .catch(() => {
        err = true;
      });
    const returnData = { err, data: data?.data };
  ...
  static async gadoRequest(image) {
    let err = false;
    const data = await request.get('/gado', { data: { image } }).catch(() => {
      err = true;
    });
    const returnData = { err, data: data?.data };
    return returnData;
