...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (!mencionado) return message.channel.send("❌ | Você deve usar give `opção` @usuario `valor`")
        ...
        if (!valor) return message.channel.send("❌ | Você não me disse o valor")
    ...
    function giveStar(user, user2, valor, message, mencionado){
        ...
        if(valor > user.estrelinhas) return message.channel.send(`❌ | ${message.author} você não tem este tanto de estrelinhas`)
    ...
    function giveDemon(user, user2, valor, message, mencionado){
        ...
        if(valor > user.caçados) return message.channel.send(`❌ | ${message.author} você não tem este tanto de demônios`)
        ...
        message.channel.send(`✅ | ${message.author} transferiu **${valor}** <:Demon:758765044443381780> para ${mencionado}`)

    }
    function giveAngel(user, user2, valor, message, mencionado){
       
        if(valor > user.anjos) return message.channel.send(`❌ | ${message.author} você não tem este tanto de anjos`)
        ...
        message.channel.send(`✅ | ${message.author} transferiu **${valor}** <:Angel:758765044204437535> para ${mencionado}`)
 
    }
    function giveSD(user, user2, valor, message, mencionado){
        
        if(valor > user.semideuses) return message.channel.send(`❌ | ${message.author} você não tem este tanto de semideuses`)
        ...
        message.channel.send(`✅ | ${message.author} transferiu **${valor}** <:SemiGod:758766732235374674> para ${mencionado}`)

    }
    function giveGod(user, user2, valor, message, mencionado){
        
        if(valor > user.deuses) return message.channel.send(`❌ | ${message.author} você não tem este tanto de deuses`)
        ...
        message.channel.send(`✅ | ${message.author} transferiu **${valor}** <:God:758474639570894899> para ${mencionado}`)
