...
import { devEnviroment, getEnviroments, prodEnviroment } from '../utils/getEnviroments.js';
import { logger } from '../utils/logger.js';

const { REDIS_PATH } = getEnviroments(['REDIS_PATH']);

const MainRedisClient = new Redis({
  db: devEnviroment ? 1 : 0,
  lazyConnect: true,
  maxRetriesPerRequest: 3,
  connectTimeout: 5_000,
  commandTimeout: 3_000,
  path: prodEnviroment ? REDIS_PATH : undefined,
  host: prodEnviroment ? undefined : process.env.REDIS_URL,
});

const VangoghRedisClient = new Redis({
  db: devEnviroment ? 2 : 6,
  lazyConnect: true,
  maxRetriesPerRequest: 3,
  connectTimeout: 5_000,
  commandTimeout: 3_000,
